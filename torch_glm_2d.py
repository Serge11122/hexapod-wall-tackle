...
class TensorEntry:
    ...
    def __recursive_iterate(obj, prefix=''):
        # print(f'{type(obj)=} {hasattr(obj, "__dict__")=}')
        if not hasattr(obj, "__dict__"):
            return
        for field, value in vars(obj).items():
            # print(f'{type(obj)=} {field=} {value=}')
    ...
    @staticmethod
    def zero_tensor(top_obj):
        i = 0
        for _ in TensorEntry.__recursive_iterate(top_obj):
            i += 1
        return torch.zeros(size=(i,))

    @staticmethod
    def get_bounds(top_obj):
        print(f'get_bounds {top_obj=}')
        lb, ub = [], []
        for obj, field, metadata, pretty_name in TensorEntry.__recursive_iterate(top_obj):
            lb.append(metadata.lb)
            ub.append(metadata.ub)
        return lb, ub

    @staticmethod
    def get_names(top_obj):
        for obj, field, metadata, pretty_name in TensorEntry.__recursive_iterate(top_obj):
            yield pretty_name
