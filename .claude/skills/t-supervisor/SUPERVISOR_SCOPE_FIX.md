...
**Solution:** Clear, strict boundary. Supervisor owns timer/orchestration. Code issues escalate to tdev.
...
Today's incident (2026-04-04 gate_unit.json missing) was a code violation that supervisor tried to work around. It should have been escalated to tdev immediately, not handled by supervisor restart/bypass.
...
| **Spin loop (file not found)** | `t2_launch_blocked_1.txt` says `FILE_NOT_FOUND: path/to/train.py` | Use Glob to find correct path, rewrite JSON | No — supervisor fixes bad paths written by tdev |

**Key:** None of these require understanding or changing project code logic.

### What tdev/tconv OWNS (Project Code)

tdev/tconv own these and supervisor ESCALATES:

| Issue | Symptom | Who Fixes | Location | Type |
|-------|---------|-----------|----------|------|
| **Missing gate marker** | tdeep finds `gate_unit.json` not written by code | tdev modifies train.py to write marker | `firstrate_learning/train.py` line ~848 | Code bug |
| **Wrong checkpoint_every** | tdeep finds `checkpoint_every=2000` but gate spec says 500 | tdev changes config or code | Train config section | Code logic |
| **Import error** | Script crashes on import (code syntax issue) | tdev fixes imports | Module file | Code bug |
| **Gate progression broken** | tdeep finds `--prove-out` launched without `gate_smoke.json` existing | tdev updates CLAUDE.md rule or launches smoke first | Policy enforcement | Code/policy |
| **Uncomm itted changes** | tdeep finds working tree dirty | tdev commits or stashes | Git state | Code ownership |
| **Conviction violation** | tdeep finds code uses epoch loops instead of step loops | tconv analyzes (via subagent), tdev implements fix | Train logic | Architectural |

**Key:** If it requires understanding the project intent, modifying .py files, or changing logic, it's tdev's job.
...
2. Classify: is this INFRASTRUCTURE or CODE?
   ...
   IF code issue:
      ...
      → Supervisor exits with "escalated to tdev"
      → On next timer tick: tdev reads escalation note, tconv re-analyzes, tdev implements fix
```
...
Classification: CODE ISSUE
  → These require tdev to modify train.py
  → Supervisor cannot know the right fix
  
Action: Escalate
  1. Write to .manager/supervisor_diagnostics.md:
     "tdeep found 2 violations: gate markers, checkpoint config. Escalating to tdev for code fix."
  2. Set timer_cycle_state.json next_step=2 to re-route to tdev
  3. Exit with message: "Code violations detected. Escalated to tdev for implementation."
  4. Next cycle: tdev runs, reads convictions + tdeep results, implements fixes
```
...
## Part 3: Escalation Pattern — How Supervisor Escalates to tdev
...
- "checkpoint_every is a training hyperparameter. Only tdev understands correct values for different gates."

## Escalation Actions Taken

1. Set timer_cycle_state.json next_step=2 (tdev)
2. Wrote this diagnostic file for tdev to read
...
Set `next_step=2` to route back to tdev (not tconv):
...
cat > .manager/timer_cycle_state.json.tmp <<EOF
{
  ...
  "escalation_reason": "Code violations detected by tdeep — requires tdev implementation"
}
EOF
...
1. Timer-dev reads `next_step=2`, sends `/tdev`
2. tdev skill reads:
...
3. tdev subagent implements code fixes
4. tdev writes new launch_commands.json
5. tdev exits; timer-dev detects IDLE
6. Loop continues: tconv → tdev → tdeep → launch
...
- Code violations found by tdeep → **escalate to tdev** (clear code issue, not supervisor's domain)
...
- Correct response: "Escalate to tdev. This is code maintenance."
...
Action: Escalate
  ...
  2. Set next_step=2 (route to tdev)
  3. Exit supervisor run
  
Next Cycle:
  1. tdev reads violations, understands they require code changes
  2. tdev subagent modifies train.py:
     - Add gate_unit.json write for run_type=="unit"
     - Change checkpoint_every to 500 for smoke
  3. tdev commits changes (7c1ecef)
  4. tdev writes new launch_commands.json
...
**Lesson:** Supervisor cannot judge whether code is "correct enough." Only tdev can decide that.
...
- t2_launch_blocked_1.txt says: `FILE_NOT_FOUND: firstrate_portfolio/v13/train.py`
- But correct path is actually: `firstrate_learning/v13/train.py` (tdev wrote wrong module path)

**Classification:** TIMER ISSUE (supervisor-fixable)

Why? Because it's a data error, not a code logic issue:
- tdev was supposed to determine correct module path
- tdev wrote it wrong (wrong module, wrong path, or missing fields)
...
Action: Fix directly
  ...
  2. Read launch_commands.json to see what tdev wrote
     module: "firstrate_portfolio.v13.train" (WRONG: portfolio not learning)
...
- tdev already wrote the command (just wrong path)
...
Why? Because tdev owns code commits:
- tdev was supposed to commit or explain why changes aren't committed
- Supervisor cannot decide if changes are correct
- Supervisor cannot commit (not tdev's role to verify correctness)
...
Classification: CODE ISSUE
  Reason: Code ownership + commit authority belongs to tdev, not supervisor.
  Supervisor cannot judge if changes are correct and safe to commit.
  
Action: Escalate
  1. Write supervisor_diagnostics.md:
     "tdeep found uncommitted .py file changes. tdev must review, commit, or stash."
  2. Set next_step=2
  3. Exit
...
2. **Is this a CODE problem?** (violations, logic bugs, gate markers, uncomm itted changes)
   ...
   - → Supervisor escalates to tdev (see Step 5e below)
...
**5e. Escalation: Code Issues to tdev**
...
2. **Update timer_cycle_state.json** to force re-route to tdev:
   ```bash
...
4. **On next timer tick:**
   ...
   - Sends /tdev to t_1_dev
   - tdev subagent reads diagnostics + violations
   - tdev implements code fixes
   - tdev commits and writes launch_commands.json
...
- tdeep found 2 violations: gate_unit.json not written, checkpoint_every=2000 (need 500 for smoke).
  ...
  Next cycle: tdev will implement fixes in train.py, commit, and cycle resumes.
...
supervisor escalates: writes diagnostic, sets next_step=2
  ↓
timer-dev sends /tdev
  ↓
tdev reads violations, implements fixes, commits code
  ↓
cycle resumes: tconv → tdev (fix applied) → tdeep (re-validate) → launch
  ↓
...
- [ ] Understand: supervisor owns TIMER issues, tdev owns CODE issues
