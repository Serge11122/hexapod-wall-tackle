...
### The 4-Step Cycle
```
tconv (Step 1) → tdev (Step 2) → tdeep (Step 3) → monitoring (Step 4) → repeat
    │                │                │                │
...
- **Steps 1-3**: Timer-dev sends `/tdevauto` **ONCE per cycle** (not once per sub-skill). Tdevauto chains tconv→tdev→tdeep internally in a single invocation, then goes IDLE. Timer-dev detects IDLE, reads output files (`launch_commands.json`, `deep_analysis_results.md`) to determine what tdevauto did, then advances state. No `/clear` is sent — context accumulates in the persistent session.
- **Step 4**: Timer-dev does NOT send any `/tdevauto` command. Step 4 is bash-only monitoring.
- **Phase flag**: `timer/data/t2_tdevauto_sent_N.txt` — written when `/tdevauto` is sent, deleted when IDLE is detected. Watchdog: 40 min (covers the full tconv+tdev+tdeep chain).
- **Tick display**: Timer shows `tdevauto(→tconv)`, `tdevauto(→tdev)`, `tdevauto(→tdeep)` depending on `next_step` at send time. Step 4 shows `monitoring`.

In legacy mode (`AUTO_MODE=false`), timer-dev sends `/clear` then the individual skill command (`/tconv`, `/tdev`, `/tdeep`) per step, using `t2_tconv_sent_N.txt`, `t2_tdev_sent_N.txt`, `t2_tdeep_sent_N.txt` flags with a 20 min watchdog each. This mode is no longer the default.

### tdevauto — Internal Routing Logic

**Timer sends `/tdevauto` ONCE per cycle.** Tdevauto chains all applicable sub-skills (tconv→tdev→tdeep) in a single invocation before going IDLE. It does NOT exit after each sub-skill and wait for the timer to re-send.

Starting route is determined by `next_step` in `timer_cycle_state.json`:

| `next_step` | Starting route | Typical chain |
|-------------|----------------|---------------|
| 1 | `/tconv` | tconv → tdev → tdeep (if launch needed) |
| 2 | `/tdev` | tdev → tdeep (if launch_commands.json written) |
| 3 | `/tdeep` | tdeep only |
| 4 | **Exit immediately** | Bash-only — no LLM work |
| unknown | `/tconv` | Safe default |

**Chaining rule after each sub-skill:**
- After tconv: if memory_dev.md has tasks → run tdev; else → exit
- After tdev: if launch_commands.json written → run tdeep (MANDATORY); else → exit
...
- If `next_step=2` AND `launch_commands.json` already exists → skip tdev, run `/tdeep` directly
- If `next_step=1` AND memory_dev.md has fresh actionable tasks → skip tconv, run `/tdev`
...
- Tdeep is the SOLE writer of `deep_analysis_results.md`. If tdeep never runs, that file is never written, and the process is never launched — regardless of what tdev put in `launch_commands.json`.
...
| `timer/tmux_timer_rev.sh N` | `timer-rev-N` | `t_N_rev` | Haiku no-think | Reviewer. /trev every 30 min. Goal velocity checks. |

### State Machine Owner: timer-dev
timer-dev OWNS timer_cycle_state.json. No t* skill writes it — not tconv, not tdev, not tdeep, not tdevauto.
...
```
timer-dev → t_N_dev:    /tdevauto (auto mode, steps 1-3) OR /tconv,/tdev,/tdeep (legacy mode)
tdevauto → tconv:       Skill tool dispatch (internal, no tmux send)
tdevauto → tdev:        Skill tool dispatch (internal, no tmux send)
tdevauto → tdeep:       Skill tool dispatch (internal, no tmux send)
timer-eta → t_N_eta:    /teta (tmux send-keys)
timer-rev → t_N_rev:    /trev (tmux send-keys)
t_N_dev → timer-dev:    launch_commands.json (tdev), deep_analysis_results.md (tdeep)
t_N_eta → timer-dev:    kill_violations.md, eta_done_N.json
tconv → tdev:           memory_dev.md
trev → memory_dev.md:   directional pivot guidance (via subagent)
```
...
| memory_dev.md | tconv, tdev, teta, trev subagent | tdev | .manager/ | Persistent, appended |
| launch_commands.json | tdev | timer-dev, tdeep | .manager/ | Archived → t2_last_launch_N.json after launch |
...
| timer_cycle_log.md | all t* skills | t-superv, trev | .manager/ | Table format, newest first |
...
| t2_spin_count_N.txt | timer-dev | timer-dev, t-superv | timer/data/ | Count of consecutive tconv→tdev cycles without LAUNCH |
...
| reviewer_report.md | trev | user | .manager/ | Periodic goal velocity report (every 30 min) |

### Two-Phase Execution Pattern (Steps 1-3)
```
Phase 1: timer-dev sends /tdevauto (auto mode) OR /tconv|/tdev|/tdeep (legacy)
...
(tdevauto chains tconv→tdev→tdeep internally; timer-dev sees ACTIVE, waits)

Phase 2: timer-dev detects IDLE → reads output files → advances state
         Reads: launch_commands.json (did tdev run?), deep_analysis_results.md (did tdeep run?)
...
### Launch Flow (Step 3)
1. Timer-dev detects `next_step=3` (launch_commands.json exists, tdev wrote it)
...
Cause: Claude session hung, compacted mid-skill, or waiting for input during tconv/tdev/tdeep chain
...
### 2. Spin Loop
Symptom: timer_cycle_log.md shows tconv→tdev→tconv repeating with no LAUNCH
Cause: tdev fails to produce valid launch_commands.json (missing script_file/module), OR tdeep blocks with VIOLATIONS
Diagnosis files: `timer/data/t2_spin_count_N.txt` (count), `timer/data/t2_launch_blocked_N.txt` (reason)
Fix: Spin counter pauses after 5 cycles. Supervisor reads both files to understand root cause. If t2_launch_blocked_N.txt says NO_SCRIPT → tdev skill output is malformed. If VIOLATIONS → tdeep found real violations. Cross-reference deep_analysis_results.md.
...
- **AUTO_MODE**: if Phase 1→Phase 2 takes >40 min (covers full tconv+tdev+tdeep chain), watchdog resets to tconv and deletes `t2_tdevauto_sent_N.txt`.
- **LEGACY_MODE**: if any individual step takes >20 min, watchdog resets to tconv.

### Spin Loop Detection
Counter increments each tconv→tdev cycle. Pauses timer after 5 without launch.
...
| t_N_dev (Sonnet) | t_N_dev | per-phase | tdevauto→tconv/tdev/tdeep rows in cycle_log | 3a: phase stuck >20 min; 3j: permission prompt |
...
| timer-rev | timer-rev-N | 30 min | trev rows in cycle_log | timer-rev dead or not ticking |
| t_N_rev (Haiku) | t_N_rev | 30 min | reviewer_report.md freshness | trev not completing; session dead |
...
- `timer/data/t2_spin_count_N.txt`: consecutive tconv→tdev cycles without LAUNCH. Value >0 = warning; >5 = pipeline self-paused.
